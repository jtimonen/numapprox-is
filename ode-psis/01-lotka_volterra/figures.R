...
lab1 <- expression(time[MCMC]^{RK45(tol)})
lab2 <- expression(time[MCMC]^{RK45(0.001)} + time[IS]^{RK45(tol)})

labs <- c(lab1, lab2)
aesth <- aes(x = logtol, y = time, group = procedure, color = procedure)
plt_A <- ggplot(df1, aesth) + geom_line() + geom_point() + theme_bw() + 
...
lab1a <- expression(time[MCMC]^{RK4(M)})
lab1b <- expression(time[MCMC]^{midpoint(M)})
lab2a <- expression(time[MCMC]^{RK4(2)} + time[IS]^{RK4(M)})
lab2b <- expression(time[MCMC]^{midpoint(3)} + time[IS]^{midpoint(M)})

labs <- c(lab1b, lab1a, lab2b, lab2a)
aesth <- aes(x = num_steps, y = time, group = group, color = group)
plt_B <- ggplot(df, aesth) + geom_line() + geom_point() + theme_bw() + 
  scale_color_brewer(type = "div", palette = 5, labels = labs) +
  xlab("Number of steps M") + 
  theme(legend.position = c(0.7, 0.45), legend.title = element_blank()) +
  scale_x_continuous(breaks=seq(2,30,by=2)) + ylim(3.5, 8)
if (ylog) {
